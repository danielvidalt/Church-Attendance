      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <div className="lg:col-span-1 space-y-3.5">
          ...
          <div className="bg-white p-5 rounded-2xl border border-slate-200 space-y-4 shadow-sm">
            <h4 className="text-xs font-black tracking-widest text-indigo-600 uppercase">
              {t.summaryOfToday}
            </h4>
            <div className="space-y-2.5 divide-y divide-slate-100">
              <div className="flex justify-between items-center pt-2">
                <span className="text-xs font-semibold text-slate-500">{t.totalPresents}:</span>
                <span className="text-lg font-black text-slate-900">{liveStats.present + anonCount}</span>
              </div>
              <div className="flex justify-between items-center pt-2">
                <span className="text-xs font-semibold text-slate-500">{t.regularPresents}:</span>
                <span className="text-sm font-black text-slate-800">{liveStats.regular}</span>
              </div>
              <div className="flex justify-between items-center pt-2">
                <span className="text-xs font-semibold text-slate-500">{t.newPresents}:</span>
                <span className="text-sm font-black text-emerald-600">{liveStats.new + anonCount}</span>
              </div>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      className="w-6 h-6 flex items-center justify-center bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-lg cursor-pointer transition-colors font-black text-sm"
                    >−</button>
                    <span className="text-lg font-black text-amber-600 min-w-[1.5rem] text-center">{anonCount}</span>
                    <button
            ...
            {matchedEvent && (
              <div className="flex items-center gap-1.5 px-2 py-1 bg-amber-50 border border-amber-100 rounded-lg mt-3">
                <Pencil className="w-3 h-3 text-amber-600 shrink-0" />
                <span className="text-[10px] font-bold text-amber-700">{t.editingRecord}</span>
              </div>
            )}

            <button
              onClick={handleSaveAttempt}
              className="w-full flex items-center justify-center gap-2 py-2.5 bg-indigo-600 hover:bg-indigo-700 active:scale-95 text-white text-xs font-bold rounded-xl shadow-sm shadow-indigo-100 transition-all cursor-pointer mt-3"
            >
            ...
            {matchedEvent && onResetCurrentEvent && (
              !resetConfirm ? (
                <button
                  ...
                  className="w-full flex items-center justify-center gap-2 py-2 bg-slate-50 hover:bg-red-50 text-slate-400 hover:text-red-500 border border-slate-200 hover:border-red-200 text-xs font-bold rounded-xl transition-all cursor-pointer mt-2"
                >
